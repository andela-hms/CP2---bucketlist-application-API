...
class RegisterAPI(Resource):
    ...
    def __init__(self):
        ...
        self.reqparse.add_argument('email', type=inputs.regex(r"[^@]+@[^@]+\.[^@]+"), \
        required=True, help="Invalid email format", location='json')
        self.reqparse.add_argument('password', type=str, required=True, \
        help='No password provided', location='json')
    ...
    def post(self):
        ...
        if not check_user:
            try:
                check_user = User(username=args['username'], \
                email_address=args['email'], password_hash=args['password'])
